  ...
  return (
    <section id="agentdojo-benchmark" className="scroll-mt-24 px-4 py-12 sm:px-6 lg:px-8 lg:py-14">
      <div className="mx-auto max-w-5xl">
        ...
        >
          <motion.span variants={fadeIn} className="font-mono text-xs font-medium uppercase tracking-[0.2em] text-blue-600/80">
            AgentDojo benchmark
          </motion.span>
          <motion.h2 variants={fadeUp} transition={{ duration: 0.6 }} className="mt-3 text-3xl font-extrabold tracking-tighter text-gray-900 sm:text-4xl lg:text-[44px] lg:leading-[1.08]">
            Red-team performance
            <br />
            <span className="text-gradient">across real agent workflows.</span>
          </motion.h2>
          <motion.p variants={fadeIn} transition={{ duration: 0.6 }} className="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-gray-600 sm:text-base">
        ...
        <motion.div
          ...
          className="mt-8 overflow-hidden rounded-2xl border border-blue-100 bg-white shadow-[0_16px_45px_-32px_rgba(37,99,235,0.35)]"
        >
              ...
              {benchmarkRows.map((row, index) => (
                <div key={row.suite} className={`rounded-xl px-2 pb-3 pt-2 ${row.suite === "Overall" ? "bg-blue-50/70" : ""}`}>
                  <div className="flex items-end justify-center gap-2.5 border-b border-gray-200 px-1">
                    <VerticalMetricBar value={row.attackSuccess} color="orange" delay={index * 0.05} />
                    <VerticalMetricBar value={row.utility} color="blue" delay={index * 0.05 + 0.08} />
                  </div>
